...
const { config1 } = require('../config')
  ...
  scenario('Register a Profile Spec and map handle field to persona', async (s, t) => {
  ...
  scenario('Can register a profile and retrieve', async (s, t) => {
    ...
    const register_result = await player1.call('chat', 'chat', 'register', {name: 'alice', avatar_url: ''})
    await s.consistency()
    // console.log(register_result)
    t.equal(register_result.Ok.length, 63)

    const get_profile_result = await player1.call('chat', 'chat', 'get_member_profile', {agent_address: register_result.Ok})
