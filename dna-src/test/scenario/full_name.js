module.exports = scenario => {
  ...
  scenario('Agent 2 requests full_name from Agent 1', async (s, t) => {
    const {player1} = await s.players({player1: config1}, true)

    // const {player1, player2} = await s.players({player1: config1, player2: config2}, false)
    // player1.spawn()
    // player1.kill()
    ...
    await player1.call('personas', 'personas', 'add_field', {persona_address: persona_address, field: {name: 'handle', data: '@philt3r'}})
    ...
    const {player2} = await s.players({player2: config2}, true)
